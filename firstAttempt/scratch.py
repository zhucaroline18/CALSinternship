# divide result values by 162.42, 3.61, 8.54, 2.824388, 42.65

import numpy as np
from csv import reader
def load_file(filename, max_rows):
    with open(filename) as file:
        datareader = reader(file)
        ret = list()
        count = 0
        for row in datareader:
            if row is None:
                continue
            ret.append(row)
            count+=1
            if (max_rows > 0 and count >= max_rows):
                break
        return ret
    
def load_dataset(filename, max_rows):
    if (max_rows > 0):
        max_rows += 1
    data = load_file(filename, max_rows)
    data = data[1:]
    train_dataset = list()
    test_dataset = list()
    train_labels = list()
    test_labels = list()
    for raw_row in data:
        var = [raw_row[0:6]]
        row = [raw_row[6:]]

        inTraining=True
        for item in row[0]:
            if item == '':
                test_dataset.append(var)
                test_labels.append(row)
                inTraining = False
        if inTraining:
            train_dataset.append(var)
            train_labels.append(row)
    return test_dataset, test_labels, train_dataset, train_labels

def print_all_data(filename):
    test_dataset, test_labels, train_dataset, train_labels = load_dataset(filename, 200)
    print(train_labels)
    print()
    print(test_labels)

def show_data_at(dataset, labels, index):
    A2 = labels[index][0]
    input = dataset[index][0]
    print('input:')
    print(f'chicken part: {"breast" if input[0] == 1 else ("thigh" if input[1] == 1 else ("liver" if input[2]==1 else "adipose"))}')
    print(f'concentration: {"0%" if input[4] == 0 else ("1%" if input[4] == 0.25 else ("2%" if input[4]==0.5 else "4%"))}')
    print(f'week: {"3" if input[5]==0 else "6"}')
    print()
    print ('ouput:')
    print(f'BCA/WEIGHT: {A2[0]}')
    print(f'MDA/WEIGHT: {A2[1]}')
    print(f'NEFA/WEIGHT: {A2[2]}')
    print(f'TC/WEIGHT: {A2[3]}')
    print(f'TG/WEIGHT: {A2[4]}')
    print()

def ReLU(Z):
    return np.maximum(Z, 0)

def ReLU_deriv(Z):
    toReturn = list()
    for row in Z:
        new_row = list()
        for col in row:
            new_col = 1 if col > 0 else 0
            new_row.append(new_col)
        toReturn.append(new_row)
    return toReturn

def softmax(Z):
    exp = np.exp(Z)
    sum = np.sum(exp)
    return exp/sum

class NeuralNetwork:
    def __init__(self, inputCount, hiddenCount, outputCount):
        self.inputCount = inputCount
        self.outputCount = outputCount
        self.hiddenCount = hiddenCount

        self.W1 = np.random.rand(hiddenCount, inputCount)-0.5
        self.b1 = np.random.rand(hiddenCount, 1)-0.5

        self.W2 = np.random.rand(outputCount, hiddenCount)-0.5
        self.b2 = np.random.rand(outputCount, 1) - 0.5

        self.learning_rate = 0.1
    

    def feed_forward(self, X):
        #X is the input 
        Z1 = self.W1.dot(X)+self.b1
        A1 = ReLU(Z1)
        Z2 = self.W2.dot(A1) + self.b2
        A2 = (Z2)
        return A1, A1, Z2, A2
    
    def backward_prop(self, X, Z1, A1, Z2, A2, Y):
        dZ2 = A2-Y
        print(A2)
        print(Y)
        print()
        dW2 = dZ2.dot(A1.T)
        db2 = dZ2
        dZ1 = self.W2.T.dot(dZ2)*ReLU_deriv(Z1)
        dW1 = dZ1.dot(X.T) #### possibly .t?
        db1 = dZ1

        self.W1 = self.W1 - self.learning_rate * dW1
        self.W2 = self.W2 - self.learning_rate * dW2
        self.b1 = self.b1 - self.learning_rate * db1
        self.b2 = self.b2 - self.learning_rate * db2

    def train (self, X, Y):
        Z1, A1, Z2, A2 = self.feed_forward(X)
        self.backward_prop(X, Z1, A1, Z2, A2, Y)

    def predict(self, X):
        Z1, A1, Z2, A2 = self.feed_forward(X)
        #self.formatOutput(X, A2)
        return A2

    def formatOutput(self, input, A2):
        '''print('input:')
        print(f'AST Presence: {input[0]}')
        print(f'Corn Presence: {'present' if input[1] == 1 else 'not present'}')
        print(f'Soybean Presence: {'present' if input[2] == 1 else 'not present'}')
        print(f'Breed: {'broiler' if input[3] == 1 else 'laying hen'}')
        print(f'Temperature: {input[5]}')
        print(f'week of trial: {input[6]}')
        print()
        print('output')
        print(f'gene expression: {A2[0]}')
        print(f'protein production of redox enzyme: {A2[1]}')
        print(f'heptic mRNA levels: {A2[2]}')
        print(f'heat shock protein 70: {A2[3]}')
        print(f'HSTF1: {A2[4]}')
        print(f'JNK1: {A2[5]}')
        print(f'SREBP1: {A2[6]}')
        print(f'DGAT2: {A2[7]}')
        print(f'GST: {A2[8]}')
        print(f'P38MAKP: {A2[9]}')
        print(f'HSP90: {A2[10]}')
        print(f'P38MAPK: {A2[11]}')
        print(f'lipid-metabolism-related genes: {A2[12]}')
        print(f'stress responsive genes: {A2[13]}')
        print(f'antioxidant genes: {A2[14]}')'''
        print('input:')
        print(f'chicken part: {"breast" if input[0] == 1 else ("thigh" if input[1] == 1 else ("liver" if input[2]==1 else "adipose"))}')
        print(f'concentration: {"0%" if input[4] == 0 else ("1%" if input[4] == 0.25 else ("2%" if input[4]==0.5 else "4%"))}')
        print(f'week: {"3" if input[5]==0 else "6"}')

        '''print ('ouput:')
        print(f'BCA/WEIGHT: {A2[0] * 162.42}')
        print(f'MDA/WEIGHT: {A2[1] * 3.61}')
        print(f'NEFA/WEIGHT: {A2[2] * 8.54}')
        print(f'TC/WEIGHT: {A2[3] * 2.824388}')
        print(f'TG/WEIGHT: {A2[4] * 42.65}')'''

        print ('ouput:')
        print(f'BCA/WEIGHT: {A2[0]}')
        print(f'MDA/WEIGHT: {A2[1]}')
        print(f'NEFA/WEIGHT: {A2[2]}')
        print(f'TC/WEIGHT: {A2[3]}')
        print(f'TG/WEIGHT: {A2[4]}')
        print()

def train_and_predict():
    #TODO: UPDATE VALUES 
    brain = NeuralNetwork(6, 128, 5)
    test_dataset, test_labels, train_dataset, train_labels = load_dataset('chicken-part-fixed.csv', 20)
    for i in range(len(train_dataset)):

        X = list(np.float_(train_dataset[i][0]))
        X = np.array(X).reshape(-1,1)
        Y = list(np.float_(train_labels[i][0]))
        Y = np.array(Y).reshape(-1,1)

        toDivideBy = np.array([162.42, 3.61, 8.54, 2.824388, 42.65]).reshape(-1, 1)
        Y2 = Y/toDivideBy
        brain.train(X, Y2)

    differenceSum = 0
    for i in range(len(test_dataset)) :
        X = list(np.float_(train_dataset[i][0]))
        X = np.array(X).reshape(-1,1)
        print(f'{i}:')
        prediction = brain.predict(X)
        Y = list(np.float_(train_labels[i][0]))
        Y = np.array(Y).reshape(-1,1)
        toDivideBy = np.array([162.42, 3.61, 8.54, 2.824388, 42.65]).reshape(-1, 1)
        Y2 = Y/toDivideBy
        sumArray = Y2-prediction
        result = np.sum(sumArray)
        exp = result*result
        differenceSum += exp

    print(f'RMSE is {differenceSum/len(test_dataset)}')
        
def train_again(brain):
    test_dataset, test_labels, train_dataset, train_labels = load_dataset('chicken-part-fixed.csv', 200)
    for i in range(len(train_dataset)):
        
        X = np.array(train_dataset[i]).reshape(-1,1)
        Y = train_labels[i]
        brain.train(X, Y)

    differenceSum = 0
    for i in range(len(test_dataset)) :
        X = np.array(test_dataset[i]).reshape(-1,1)
        prediction = brain.predict(X)
        sumArray = np.array(test_labels[i]).reshape(-1,1)-prediction
        result = np.sum(sumArray)
        exp = result*result
        differenceSum += exp

    print(f'RMSE is {differenceSum/len(test_dataset)}')

if __name__ == "__main__":
    #brain, w1, w2, b1, b2 = train_and_predict()
    #train_again(brain)
    #test_dataset, test_labels, train_dataset, train_labels= load_dataset('chicken-part-fixed.csv', 200)
    #print(train_labels)
    #show_data_at(dataset, labels, 77)
    #print_all_data('chicken-part-fixed.csv')
    train_and_predict()
    '''x= (np.array(train_labels[0]).reshape(-1,1))
    x = list(np.float_(x))
    y = np.array([2, 1, 2, 1, 2]).reshape(-1, 1)
    print(x)
    print(y)
    print(x/y)'''
