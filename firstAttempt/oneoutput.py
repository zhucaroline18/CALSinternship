...
def show_data_at(dataset, labels, index):
    ...
    print('input:')
    print(f'chicken part: {"breast" if input[0] == 2 else ("thigh" if input[1] == 2 else ("liver" if input[2]==2 else "adipose"))}')
    print(f'concentration: {"0%" if input[4] == 0 else ("1%" if input[4] == 0.25 else ("2%" if input[4]==0.5 else "4%"))}')
    print(f'week: {"3" if input[5]==1 else "6"}')
    print(f'MDA/WEIGHT: {input[6]}')
    print(f'NEFA/WEIGHT: {input[7]}')
    print(f'TC/WEIGHT: {input[8]}')
...
class NeuralNetwork:
    ...
    def formatOutput(self, input, A2):
        ...
        print(f'MDA/WEIGHT: {input[7] }')
        print(f'NEFA/WEIGHT: {input[8] }')
        print(f'TC/WEIGHT: {input[9]}')
        '''print ('ouput:')
